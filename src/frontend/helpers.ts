export const updateQrCode = (link: string) => {
  const qrCodeNode = document.getElementById('sdk-qrcode-container');
...
export const truncateString = (value: string, length = 40, separator = '...', front = 8, back = 5) => {
  if (value.length < length) {
    return value;
  }

  return `${value.substring(0, front)}${separator}${value.substring(value.length - back)}`;
}